...
apply_transformations() {
	...
	if [[ -n ${CONVERT_EFFECT} ]]; then
		convert "$img" $CONVERT_EFFECT "$HOME/Pictures/gowall/convert_$(basename "$img")" >&2
		transformations+=" convert($CONVERT_EFFECT)"
		img="$HOME/Pictures/gowall/convert_$(basename "$img")"
	fi
...
process_wallpaper() {
	...
	"wayland")
		if pgrep -x "hyprpaper" &>/dev/null; then
			...
			if [ -z "$NIRI_NAMESPACE" ]; then
				...
				awww img "$img" -n $NIRI_NAMESPACE --transition-type wipe >/dev/null 2>&1
	...
	*)
	...
	if [ ! -z ${GENJUTSU+x} ]; then
		exit 0
	fi
	if [ ! -z ${NIRI_SOCKET+x} ]; then
		GO_WALL=nix GENJUTSU=1 NIRI_NAMESPACE=backdrop CONVERT_EFFECT=' -filter Gaussian -blur 0x8 ' bg.sh last
	fi
